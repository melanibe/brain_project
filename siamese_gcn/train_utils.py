...
""" This file defines the training loop, a single training step and a single validation step.
This is the main helper file for the fit/predict functions in the GCN_estimator file. 
"""
...
def training_step(gcn, data, optimizer, criterion):
    ...
    Args:
        ...
        data(dataframe): one batch of data (outfrom ToTorchDataset) i.e. dataframe
                with 6 columns one for the labels and one contains the 
                compressed array of the coherence matrix for each frequency
                band.
    ...
    while(train_step<nsteps):
        for data in trainloader:
            if(train_step<nsteps):
                current_loss, current_balacc = training_step(gcn, data, optimizer, criterion)
                train_step+=1
                losses += [current_loss]
                train_bal_acc += [current_balacc]
            # Display the training and validation loss every 5 steps
            if (train_step%5==0):
                logger.debug("Training loss for step %d is %.3f" % (train_step, current_loss))
                logger.debug("Training balanced accuracy for step %d is %.3f%%" % (train_step, 100*current_balacc))
                if X_val is not None:
                    loss_val_e, acc_e, bal_e = val_step(gcn, valloader, criterion, logger)
