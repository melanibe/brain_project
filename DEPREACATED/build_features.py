...
def prepare_X_bands():
    ...
    np.save(cwd+"/matrices/X_delta", np.reshape(X[:,:,0:3], (n,-1))) #1 to 3 Hz
    np.save(cwd+"/matrices/X_theta",np.reshape(X[:,:,3:7], (n,-1))) #4 to 7
    np.save(cwd+"/matrices/X_alpha", np.reshape(X[:,:,7:12], (n,-1))) # 8 to 12
    np.save(cwd+"/matrices/X_beta", np.reshape(X[:,:,12:30], (n,-1)))  # 13 to 30
    np.save(cwd+"/matrices/X_gamma", np.reshape(X[:,:,30:], (n,-1)))  #over 31
    return(1)
...
def prepare_X_mine(index_list = [0,5,10,15], channels_per_freq=1000):
    ...
    """
    try:
       X_new = np.load(cwd+"/matrices/X_mine_{}.npy".format(channels_per_freq))
       ...
       return(X_new, Y)
        ...
        np.save(cwd+"/matrices/X_mine_{}".format(channels_per_freq), X_new) 
        return(X_new, Y)
...
def prepare_X_std_freq():
    ...
    np.save(cwd+"/matrices/std", X_aggregated)
...
def prepare_X_std_freq_channels(channels_per_freq=200):
    ...
    """
    try:
        X_new = np.load(cwd+"/matrices/X_std_{}channels.npy".format(channels_per_freq))
        Y = np.load(cwd+"/matrices/y.npy")
        return (X_new,Y)
    except:
        print("Have to prepare X_std_{}channels.npy".format(channels_per_freq))
        try:
            X_aggregated = np.load(cwd+"/matrices/X_std.npy")
            Y = np.load(cwd+"/matrices/y")
        except:
        ...
        print(np.shape(X_new)) 
        np.save(cwd+"/matrices/X_std_{}channels".format(channels_per_freq), X_new)
        return (X_new,Y)
...
def augment(X, Y, save=False, name=None):
    ...
    if save:
        np.save(cwd+'/matrices/{}_aug'.format(name), X_new)
        np.save(cwd+'/matrices/Y_aug', Y_new)
    return(X_new, Y_new)
